...
// !this->client_ptr_->wait_for_action_server()
  ...
  RCLCPP_INFO(this->get_logger(), "Sending %zu poses:",
              gps_waypoint_follower_goal_.gps_poses.size());

  for (auto pose : gps_waypoint_follower_goal_.gps_poses) {
    RCLCPP_INFO(this->get_logger(), "\t(%lf, %lf)", pose.position.latitude,
  ...
  goal_options.result_callback =
      std::bind(&GPSWayPointFollowerClient::resultCallback, this, std::placeholders::_1);

  auto future_goal_handle =
      gps_waypoint_follower_action_client_->async_send_goal(
...
GPSWayPointFollowerClient::loadGPSWaypointsFromYAML(
  ...
  std::vector<std::string> waypoints_vector =
      this->get_parameter(current_path + "." + "waypoints").as_string_array();
  std::vector<geographic_msgs::msg::GeoPose> gps_waypoint_msg_vector;
  for (auto &&curr_waypoint : waypoints_vector) {
    try {
      this->declare_parameter(current_path + "." + curr_waypoint,
                              std::vector<double>({0}));
      std::vector<double> gps_waypoint_vector =
          this->get_parameter(current_path + "." + curr_waypoint)
              .as_double_array();
...
int main(int argc, char const *argv[]) {
  ...
  auto gps_waypoint_follower_client_node = std::make_shared<
      husky_gps_navigation::GPSWayPointFollowerClient>();
